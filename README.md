...
## Quickstart
1) Copy `.env.local.example` to `.env.local` and set your values (single docker repo, HF bucket, git repo, optional remote hosts/CIDRs, etc.).
...
6) On the GPU host: run containerized batch (mount `/media` and `/opt/hf-cache`), e.g. `make docker-batch IMAGE=docker.io/you/whisperx:latest` (or `make batch` locally if you have a GPU).
...
## Notes
- HF cache and media are ignored; keep `.env.local` out of git.
